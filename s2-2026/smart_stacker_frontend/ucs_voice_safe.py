#!/usr/bin/env python3
"""
Enhanced Voice-Enabled UCS with Safety Checks
==============================================
...
NEW: Safety validation to prevent animals from occupying the same position
...
#    def __init__(self, broker_host="localhost", broker_port=1883):
    ...
    def validate_command_safety(self, command_json):
        """
        Validate that command does not send multiple animals to the same position.
        Returns (is_safe, safe_command)
        - If safe: (True, command_json)
        - If unsafe: (False, current_positions) - no-op command
        """
        # Extract target positions
        target_positions = list(command_json.values())

        # Check for duplicate positions
        if len(target_positions) != len(set(target_positions)):
            print("⚠️  Model generated unsafe target positions")
            print(f"   Attempted command: {command_json}")
            print(f"   Collision detected - multiple animals assigned to same position")
            return False, self.current_positions.copy()
            ...
            with suppress_audio_errors():
                try:
                    ...
                    if "robot" in text:
                        ...
                        # Process the command with safety validation
                        command_json = self.llm.process_natural_language_command(command_text, self.current_positions)
                        is_safe, safe_command = self.validate_command_safety(command_json)

                        if is_safe:
                            self.send_command(safe_command)
    ...
    def wait_for_completion(self):
        ...
        # Wait 1 second for final position update from stacker controller
        print("   ⏳ Syncing final positions...")
        time.sleep(1)

        # Wait for position update from stacker/positions
        self.position_update_received = False
        timeout = 0
        while not self.position_update_received and timeout < 20:  # 2 second timeout
            time.sleep(0.1)
            timeout += 1
    ...
    def run_hybrid_demo(self):
        ...
        print("=" * 60)
        print("🎤 HYBRID VOICE-ENABLED ROBOT CONTROL (SAFE MODE)")
        ...
        while True:
            try:
                ...
                if is_safe:
                    ...
                    # Unsafe command - skip execution and continue to next command
                    print("   Command rejected - no animals will be moved")
                    continue
...
if __name__ == "__main__":
    print("🎤 Initializing Hybrid Voice Robot Control (Safe Mode)...")
