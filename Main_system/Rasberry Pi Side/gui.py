try:
    ...
    GUI_AVAILABLE = False
    print("[WARNING] Tkinter not found. Running in Headless Mode.")

from serial_comm import send_command, close_serial
...
def create_gui(status_callback=None):
    if not GUI_AVAILABLE:
        # --- Headless Mode ---
        def update_positions_headless(src=None, dst=None, moves=None, assign=None):
            if assign:
                animal, pos = assign
            ...
            # Log current state
            print(f"[HEADLESS] Positions Updated: {positions}")
            if status_callback:
        ...
        class HeadlessRoot:
            ...
            def mainloop(self):
                print("[HEADLESS] Main loop started. Press Ctrl+C to exit.")
                try:
                    while True:
                        time.sleep(1)
                except KeyboardInterrupt:
                    print("[HEADLESS] Exiting...")
            def destroy(self): pass
            def protocol(self, *args): pass

        return HeadlessRoot(), update_positions_headless, positions
