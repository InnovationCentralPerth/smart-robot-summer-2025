...
float POS_A[6] = { 0, 100, 130, 75, 140, 0 };
float POS_B[6] = { 0, 100, 130, 75, 140, 50 };
float POS_C[6] = { 0, 100, 130, 75, 140, 100 };
float POS_D[6] = { 0, 100, 130, 75, 140, 150 };
float POS_E[6] = { 0, 100, 130, 75, 140, 200 };
float POS_F[6] = { 0, 100, 130, 75, 140, 250 };

// Top positions (safe travel heights)
//{ 0, 100, 157, 157, 157, 0 }
const float FIXED_TOP[6] = { 0, 100, 140, 140, 140, 0 };
float TOP[6] = { 0, 100, 140, 140, 140, 0 };  // claw value filled dynamically //clamp (no use), clamp rotate (no use), top of it, third from bottom,NOO DON"T EVER MOVE, bottom base 0, 100, 140, 140, 140, 0
...
void setup() {
  ...
  if (Braccio.begin()) {
    ...
    moveArm(TOP, 180); //default position
  }
}
  ...
  // Single letters (A-F) or special commands
    ...
    switch (c) {
      ...
      case 'L': { // top sequence loop
        ...
        bool exitLoop = false;
        int sequence[6][6] = {
          { 0, 100, 140, 140, 140, 50 },
          { 0, 100, 130, 75, 140, 50 },
          { 0, 100, 140, 140, 140, 100 },
          { 0, 100, 130, 75, 140, 100 },
          { 0, 100, 140, 140, 140, 150 },
          { 0, 100, 130, 75, 140, 150 }
        };
        while (!exitLoop) {
          for (int i = 0; i < 6; i++) {
            Braccio.moveTo(sequence[i][0], sequence[i][1], sequence[i][2], sequence[i][3], sequence[i][4], sequence[i][5]);
