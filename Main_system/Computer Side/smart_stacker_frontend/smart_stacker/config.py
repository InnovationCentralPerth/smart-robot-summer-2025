...
class LLMConfig:
    ...
    # OpenAI API key is loaded from environment
    api_key: str = os.getenv("OPENAI_API_KEY", "")
    # Model to use (defaulting to an OpenAI LLM)
    model_name: str = os.getenv("MODEL", "gpt-4o-mini")
    # Optional system prompt file for custom instructions
    prompt_file: Path = Path(os.getenv("PROMPT_FILE", "llm_system_prompt.txt"))
    # Log file for LLM requests/responses
    log_file: Path = Path(os.getenv("LOG_FILE", "default.log"))
    # Optional base URL (used if you run a local OpenAI-compatible server)
    host_url: str = os.getenv("HOST_URL", "https://api.openai.com/v1")
