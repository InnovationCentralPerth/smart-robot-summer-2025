...
class HybridVoiceController:
    ...
    def continuous_mode(self) -> None:
        ...
        print(f"👂 Listening for wake word: '{self.speech.config.wake_word}'...")
    ...
    def _dispatch(self, command: Dict[str, str]) -> CommandResult:
        ...
        success = self.mqtt_client.wait_for_completion()

        
        if success:
            return CommandResult(command=command, is_safe=True, message="Command executed")
        else:
            return CommandResult(command=self.current_positions.copy(), is_safe=False, message="Command timed out - Robot not responding")
